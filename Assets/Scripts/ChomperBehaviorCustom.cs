...
namespace Gamekit3D
{
    ...
    public class ChomperBehaviorCustom : ChomperBehavior, IMessageReceiver
    {
        GameObject vrPlayerRig = null;
        GameObject m_vrTarget = null;

        float heightOffset = 0.0f;

        float detectionRadius = 10;

        float detectionAngle = 270;
        ...
        public GameObject Detect(Transform detector, bool useHeightDifference = true)
        {
            ...
            Vector3 toPlayer = vrPlayerRig.transform.position - eyePos;
            Vector3 toPlayerTop = vrPlayerRig.transform.position + Vector3.up * 1.5f - eyePos;
        ...
        public void FindVrTarget()
        {
            //we ignore height difference if the target was already seen
            GameObject target = Detect(transform, m_vrTarget == null);
            ...
            else
            {
                ...
                if (target == null)
                {
                    ...
                    {
                        Vector3 toTarget = m_vrTarget.transform.position - transform.position;
